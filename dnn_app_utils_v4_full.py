...
"""
Este cÃ³digo esta basado en el curso de DeepLearning del profesor Andrew Ng
"""
...
def initialize_parameters_he(layers_dims):
    ...
    for l in range(1, L):
        parameters['W' + str(l)] = np.random.randn(layers_dims[l-1],layers_dims[l]) * ((2/layers_dims[l-1])**(0.5))
        parameters['b' + str(l+1)] = np.zeros((layers_dims[l], 1))
        ### END CODE HERE ###
    print (parameters['W2'])
...
def predictNClasses(X, y, parameters):
    """
    ...
    print("Accuracy: "  + str(np.sum((np.argmax(probas, axis=0) == np.argmax(y, axis=0)))/m))
